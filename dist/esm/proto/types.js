...
const NostrSignResponseValidate = (o, opts = {}, path = 'NostrSignResponse::root.') => {
    ...
    if (typeof o !== 'object' || o === null)
        return new Error(path + ': object is not an instance of an object or is null');
    if (typeof o.signed_event !== 'string')
        return new Error(`${path}.signed_event: is not a string`);
    if (opts.signed_event_CustomCheck && !opts.signed_event_CustomCheck(o.signed_event))
        return new Error(`${path}.signed_event: custom check failed`);
    return null;
};
